...
# Нововведение 13.2:
Программа предназначена для чтения файлов транзакции,
пользователь теперь не может сам вводить транзакцию. Транзакции находятся в директории data.
```
...
Вы можете выбрать один из вариантов операций. 
Далее вас запросят ввести вид сортировки 
```
...
Далее программа предложит больше фильтров сортировки транзакций, следуйте указаниям терминала.
В конце программа вам выдаст данные в терминал. 
...
# Новые функции (Основные):
 При старте программы у вас появится окно:
```
Привет! Добро пожаловать в программу работы с банковскими транзакциями.
Выберите необходимый пункт меню:
1. Получить информацию о транзакциях из JSON-файла
...
```
Далее следуйте инструкции по ходу программы, главное вводите те команды, которые предлагает терминал.
Функции по транзакциям не доступны(Те что указаны сверху) --> модуль работы функции masks.py
